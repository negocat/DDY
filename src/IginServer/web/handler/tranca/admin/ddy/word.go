...
func word(c *mygin.IContext) {
	...
	c.HTML("tranca/admin/ddy/word.html", map[string]interface{}{
		"request": c.Request,
		"class":   class,
		"words":   words,
		"total":   total,
...
func wordinfo(c *mygin.IContext) {
	info := c.GetSession("admin_info").(map[string]string)
...
func updateword(c *mygin.IContext) {
	info := c.GetSession("admin_info").(map[string]string)
